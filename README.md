...
**综合监控网页**（http://localhost:8080，0.5 秒刷新）：

- 顶部四卡：🌱 土壤湿度（含进度条）/ 💧 流速（脉冲 + 累计）/ 🚰 水泵（开关 + 滑块调速 + 功率）/ 🌬️ 气象（风速 + 温湿度）
- 全宽实时曲线：温度（红，左轴）/ 湿度（蓝，右轴）/ 风速（绿），5 分钟滚动窗口
- 底部预警卡：泥石流 D / 山洪 F / 土湿 S̄ / 流量 Q̄ / 突变 dQ̄ / 土湿涨 ΔS̄ / 尖峰风险 / 风速风险
...
**串口数据**（115200，每秒一行 JSON，供网页/上位机解析）：

```json
{"pump":1,"speed":242,"soil_raw":679,"soil_percent":34,"soil_dry":true,"soil_connected":true,"temp":23.5,"hum":56,"dht_ok":true,"flow_pulses":8,"flow":0.14,"total":1.465,"wind_raw":29,"wind_kmh":14.2,"wind_ms":3.93}
```
